...
public class ScrollHandler {
    ...
    public boolean collides(Airplane airplane){
        if (!pipe1.isScored() && pipe1.getX() + (pipe1.getWidth()/2) < airplane.getX() + airplane.getWidth()){
            addScore(1);
            pipe1.setScored(true);
            AssetLoader.coin.play();
        } else if (!pipe2.isScored() && pipe2.getX() + (pipe2.getWidth()/2) < airplane.getX() + airplane.getWidth()){
            addScore(1);
            pipe2.setScored(true);
            AssetLoader.coin.play();
        } else if (!pipe3.isScored() && pipe3.getX() + (pipe3.getWidth()/2) < airplane.getX() + airplane.getWidth()){
            addScore(1);
            pipe3.setScored(true);
            AssetLoader.coin.play();
        }

        return(pipe1.collides(airplane) || pipe2.collides(airplane) || pipe3.collides(airplane));
    }
